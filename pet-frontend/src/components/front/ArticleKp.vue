      ...
      <div v-for="item in tableData" style="padding: 20px; border-radius: 10px; background-color: #fff; margin: 10px 0" :key="item.id">
        <div style="display: flex">
          ...
          <div style="width: 200px; text-align: center">
            <el-image :src="$store.state.baseApi+item.img" style="width: 100px; height: 100px" :preview-src-list="[$store.state.baseApi + item.img]"></el-image>
          </div>
...
export default {
  ...
  methods: {
    ...
    exp() {
      window.open(this.$store.state.baseApi+"/articleKp/export")
    },
